...
app.post('/api/create-post', async (req, res) => {
    ...
    try {
        ...
        if (Array.isArray(req.body.blocks)) {
            ...
            req.body.blocks.forEach((block, index) => {
                console.log(`   [${index}] ${block.type}: "${block.content?.substring(0, 50)}${block.content?.length > 50 ? '...' : ''}"`, block.metadata ? `(metadata: ${Object.keys(block.metadata).join(', ')})` : '');
            });
        }
