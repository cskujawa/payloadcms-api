...
cleanup_system() {
    ...
    if [ -d "./data/payloadcms/componentFixes" ]; then
        mkdir -p "$TEMP_DIR/componentFixes"
        cp -r "./data/payloadcms/componentFixes/"* "$TEMP_DIR/componentFixes/" 2>/dev/null || true
    fi
    if [ -d "./data/payloadcms/tools" ]; then
        mkdir -p "$TEMP_DIR/tools"
        cp -r "./data/payloadcms/tools/"* "$TEMP_DIR/tools/" 2>/dev/null || true
    fi
    if [ -d "./data/payloadcms/toolsDependencies" ]; then
        mkdir -p "$TEMP_DIR/toolsDependencies"
        cp -r "./data/payloadcms/toolsDependencies/"* "$TEMP_DIR/toolsDependencies/" 2>/dev/null || true
    fi
    if [ -d "./data/payloadcms/toolsConfig" ]; then
        mkdir -p "$TEMP_DIR/toolsConfig"
        cp -r "./data/payloadcms/toolsConfig/"* "$TEMP_DIR/toolsConfig/" 2>/dev/null || true
    fi
    if [ -d "./data/payloadcms/toolsFixes" ]; then
        mkdir -p "$TEMP_DIR/toolsFixes"
        cp -r "./data/payloadcms/toolsFixes/"* "$TEMP_DIR/toolsFixes/" 2>/dev/null || true
    fi
    ...
    if [ -d "$TEMP_DIR/componentFixes" ]; then
        mkdir -p "./data/payloadcms/componentFixes"
        cp -r "$TEMP_DIR/componentFixes/"* "./data/payloadcms/componentFixes/" 2>/dev/null || true
    fi
    if [ -d "$TEMP_DIR/tools" ]; then
        mkdir -p "./data/payloadcms/tools"
        cp -r "$TEMP_DIR/tools/"* "./data/payloadcms/tools/" 2>/dev/null || true
    fi
    if [ -d "$TEMP_DIR/toolsDependencies" ]; then
        mkdir -p "./data/payloadcms/toolsDependencies"
        cp -r "$TEMP_DIR/toolsDependencies/"* "./data/payloadcms/toolsDependencies/" 2>/dev/null || true
    fi
    if [ -d "$TEMP_DIR/toolsConfig" ]; then
        mkdir -p "./data/payloadcms/toolsConfig"
        cp -r "$TEMP_DIR/toolsConfig/"* "./data/payloadcms/toolsConfig/" 2>/dev/null || true
    fi
    if [ -d "$TEMP_DIR/toolsFixes" ]; then
        mkdir -p "./data/payloadcms/toolsFixes"
        cp -r "$TEMP_DIR/toolsFixes/"* "./data/payloadcms/toolsFixes/" 2>/dev/null || true
    fi
