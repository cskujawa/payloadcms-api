...
export class ConfigManager {
   ...
   */
  private static readEnvFile(): string | null {
    try {
   ...
   */
  private static detectFromDockerCompose(): string | null {
    try {
   ...
   */
  private static detectServerIp(): string {
    try {
   ...
   */
  static async testPayloadCMSConnection(baseUrl?: string): Promise<boolean> {
    try {
   ...
   */
  static getDefaultCredentials(): { email: string; password: string } {
    return {
   ...
   */
  static async validateConfig(config: PayloadCMSConfig): Promise<{
    valid: boolean;
    errors: string[];
    warnings: string[];
  }> {
