...
export class JSONProcessor {
     ...
     */
    static processPostCreationRequest(request) {
     ...
     */
    static convertBlocksToLexical(blocks) {
        const children = [];
        blocks.forEach(block => {
            switch (block.type) {
                ...
                case 'paragraph':
                    ...
                    break;
                case 'code':
                    children.push(this.createCodeBlock(block.content, block.metadata?.language || 'javascript', block.metadata?.blockName || 'Code Example'));
                    break;
                case 'banner':
                    children.push(this.createBannerBlock(block.content, block.metadata?.style || 'info', block.metadata?.blockName || 'Important Note'));
                    break;
                case 'mediaBlock':
     ...
     */
    static createCodeBlock(code, language, blockName) {
     ...
     */
    static createBannerBlock(text, style, blockName) {
        return {
            type: 'block',
            fields: {
                blockType: 'banner',
     ...
     */
    static createMediaBlock(blockName) {
        return {
            type: 'block',
            fields: {
     ...
     */
    static validatePostCreationRequest(data) {
        const errors = [];
